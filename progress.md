...
- **Shared tier-grouping helpers** (single source of truth, used by the tier table render, the CSV export, and the distribution table's tier rows):
  ...
  - `groupSkillsByTier(skills, skillRarity)` — buckets skills into a `Map<tierId, sortedSkills[]>` in `RARITY_TIERS` order, alphabetical within each tier. `renderTierTable` consumes it directly; `sortedTierSkills` (CSV) is just `[...groupSkillsByTier(...).values()].flat()`. Replaced the former hand-rolled `byTier` Map and the separate `TIER_INDEX` sort, so the on-screen table and the CSV now bucket identically
